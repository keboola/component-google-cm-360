...
from configuration import FILE_JSON_LABELS, Configuration, InputVariant
...
class Component(ComponentBase):
                    ...
                    if first := next(result, None):
                        if not writer:
                            table_def = self.create_out_table_definition(
                                name=f"metadata_{endpoint}.csv", primary_key=["profile_id", "id"]
                            )
    ...
    def _create_date_range(self) -> dict:
        if self.cfg.time_range.period == "CUSTOM_DATES":
            ...
            date_range = {"relativeDateRange": self.cfg.time_range.period, "startDate": None, "endDate": None}
        return date_range
    ...
    def _retrieve_table_from_raw(self, profile_id, profile_name, report_id) -> list:
        ...
        with open(in_file) as src, open(out_file, "w") as tgt:
    ...
    def _assign_profile_names(self, report_files: list):
        ...
        for report_file in report_files:
            report_file["profile_name"] = (
                profiles_2_names[report_file["profile_id"]]
                if report_file["profile_id"] in profiles_2_names
                else report_file["profile_id"]
            )
            pass
    ...
    def _write_common_manifest(self, dimensions, metrics):
        pks = (
            ...
            else self.cfg.destination.primary_key_existing
        )
    ...
    def _wait_process_report_files(self, wait_files):
        ...
        for report_file in wait_files:
            profile_id, report_id, file_id = report_file["profile_id"], report_file["report_id"], report_file["file_id"]
    ...
    def _process_generated_reports(self) -> list[dict[str, str]]:
        ...
        for profile_id in self.cfg.profiles:
            ...
            if existing_report_def:
                current_report_id = self._update_existing_report(profile_id, existing_report_def, report_definition)
            else:
        ...
        for rep_ids in self.cfg.existing_report_ids:
            ...
            if not self.common_report_type:
                ...
                if self.common_report_type != report_spec.report_type:
                    errors.append(
                        f"Missmatch in report type {self.common_report_type} x {report_spec.report_type} "
                        f"for profile {profile_id} / report {report_id}"
                    )

                if self.common_dimensions != report_spec.get_dimensions_names():
                    errors.append(
                        f"Missmatch in report dimensions {self.common_dimensions} x "
                ...
                if self.common_metrics != report_spec.get_metrics_names():
                    errors.append(
                        f"Missmatch in report metrics {self.common_metrics} x "
            ...
            if report_spec.report_representation.get("format", "CSV") != "CSV":
                errors.append(
                    f"Missmatch in report format {report_spec.report_representation.get('format')} "
                    f"for profile {profile_id} / report {report_id}"
                )
    ...
    def _update_existing_report(
        self, profile_id: str, existing_report: CsvReportSpecification, report_definition: CsvReportSpecification
    ) -> str:
        """
        ...
        self.google_client.update_report(
            report=updated_report_body, profile_id=profile_id, report_id=existing_report.report_id
        )
    ...
    def _get_existing_report_for_profile(self, profile_id: str) -> CsvReportSpecification:
        ...
        if existing_report_id:
            report_response = self.google_client.get_report(
                profile_id=profile_id, report_id=existing_report_id, ignore_error=True
            )
    ...
    def _get_report_definition(self) -> CsvReportSpecification:
        ...
        if self.cfg.input_variant == "report_specification":
            ...
            report_def = CsvReportSpecification.custom_from_specification(
    ...
    def load_reports(self):
        ...
        for profile_id in profile_ids:
            ...
            reports_w_labels.extend(
                [
                    SelectElement(
                        value=f"{profile_id}:{report['id']}",
                        label=f"[{profiles_2_names[profile_id]}] {report['name']} ({profile_id}:{report['id']})",
                    )
                    for report in reports
                    if _check_format_existing_report(report)
                ]
            )
